...
def plot_correlation_analysis(
    ...
    if add_summary_subtitle:
        ...
        if title is None:
            title = f"{latex_nonref} win rate: {nonref_win_pct:.1%}"
        else:
            title = f"{title}\n{latex_nonref} win rate: {nonref_win_pct:.1%}"
    ...
    if facet_col is not None:
        ...
        if title is not None:
            ...
            if title_x is None:
                ...
                title_x_pos = title_x
            g.fig.suptitle(title, ha=title_ha, x=title_x_pos)
        ...
        if show_category_column:
            ...
            if title is not None:
                ...
                ax_bar.set_title(title, ha=title_ha, x=title_x_pos)
            ...
            if title is not None:
                ...
                # x=0 in axes coordinates aligns with left edge of axes
                title_x_pos = 0.0 if title_x is None else title_x
                plt.title(title, ha=title_ha, x=title_x_pos)
    ...
    for row_idx, af_col in enumerate(af_cols):
        ...
        for col_idx, vep_col_item in enumerate(vep_cols):
            ...
            if fit_line:
                ...
                fit_curve_and_plot(x_fit, y_fit, ax_current, fit_model, alpha, show_ci)
            ...
            # Set title with af_col vs vep_col format
            ax_current.set_title(f"{af_col} vs. {vep_col_item} (variants={df_subset_clean['site'].nunique()})")
